...
-- StarterGui.Erestive.MainFrame.CommandFrame.Frame5.ONOFF.Slider.Button"AspectRatio"] = 1.556;
...
local script = G2L["3b4"];
	...
	-- ФУНКЦИЯ КЛИКА (Добавлен альтернативный метод)
	local function doFire()
		if mouse1click then 
			mouse1click() 
		else
			local VIM = game:GetService("VirtualInputManager")
			-- Кликаем ровно в центр экрана
	...
	local function canSee(target)
		...
		rayParams.FilterDescendantsInstances = {player.Character, target.Parent}
	...
	RunService.RenderStepped:Connect(function()
				...
				if p.Character.Humanoid.Health > 0 and (p.Team ~= player.Team or p.Team == nil) then
					...
					if part then
						...
						if onScreen then
							...
							if dist <= radius and canSee(part) then
								targetFound = true -- Враг в прицеле!
		...
		-- МЕНЯЕМ ЦВЕТ КРУГА
		FOVCircle.Color = targetFound and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(0, 255, 255)
	end)
